...
export default function ConsolidatedEvaluationSummaryPage({ rows, staffNames, branches, periods, request }: PageProps) {
    ...
    const openDetails = async (evaluatorId: string, evaluatorName: string, staffName: string, row: any) => {
        if (row[staffName] === null || row[staffName] === '-') return
        ...
        try {
            ...
            const res = await fetch(`/reports/regional-production-maintenance-summary/details?${params.toString()}`, {
                headers: {
    ...
    const applyFilters = () => {
        window.location.href = `/reports/regional-production-maintenance-summary${buildQuery()}`
    }

    const visibleStaffNames = React.useMemo(() => {
        if (!staffFilter || staffFilter === 'all') {
            return staffNames
        }
        return staffNames.filter((name: string) => name === staffFilter)
    ...
    return (
        <EvaluationTabsLayout activeTab="consolidated" title="Regional , Production & Maintanance Managers Evaluation" breadcrumbs={[]}>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="flex gap-4 items-end flex-wrap">
                            ...
                            <div className="w-56">
                                <label className="text-sm font-medium mb-2 block">Staff Columns</label>
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    ...
                                    if (periodId) params.set('period_id', periodId)
                                    if (visibleStaffNames.length > 0 && visibleStaffNames.length < staffNames.length) {
                                        params.set('columns', visibleStaffNames.join(','))
                                    }
                                    window.location.href = `/reports/regional-production-maintenance-summary/export${params.toString() ? `?${params.toString()}` : ''}`
                                }}
                            >
                                Export CSV
                            </Button>
                    ...
                    <CardContent className="p-0">
                        <Table>
                            <TableHeader className="bg-slate-500 dark:bg-slate-700 sticky top-0 z-10">
                                <TableRow>
                                    <TableHead className="font-bold text-white whitespace-nowrap">Evaluator Branch</TableHead>
                                    <TableHead className="font-bold text-white whitespace-nowrap">Evaluator Name</TableHead>
                                    {visibleStaffNames.map((name: string) => (
                                        <TableHead key={name} className="font-bold text-white text-center whitespace-nowrap" title={`Staff: ${name}`}>
                                            {name}
                                        </TableHead>
                                    ))}
                                </TableRow>
                            ...
                            <div className="space-y-6">
                                <div className="flex flex-col gap-2 bg-slate-50 dark:bg-slate-800 p-4 rounded-lg">
                                    <div className="flex items-center gap-2">
                                        <span className="font-semibold text-slate-600 dark:text-slate-400">Evaluatee:</span>
