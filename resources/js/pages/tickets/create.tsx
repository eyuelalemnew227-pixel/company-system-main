...
export default function TicketCreate({ departments, mainCategories, subCategories, assets, severities, products, purchaseRequestCatId, purchaseDeptId, flash, isSparePartRequest, parentTicketId, sparePartCategories, spareParts, branches = [], allDepartments = [], fiscalYears = [], fiscalMonths = [] }: PageProps) {
  ...
  useEffect(() => {
    if (!data.fiscal_year_id) {
      if (data.fiscal_month_id) setData('fiscal_month_id', '');
      return;
    }
    if (data.fiscal_month_id && !filteredFiscalMonths.find((m) => String(m.id) === data.fiscal_month_id)) {
      setData('fiscal_month_id', '');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data.fiscal_year_id, filteredFiscalMonths]);
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              {isPurchaseMode && (
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <Field label="Fiscal Month" error={(errors as any).fiscal_month_id}>
                    <Select
                      value={data.fiscal_month_id}
                      onValueChange={(v) => setData('fiscal_month_id', v)}
