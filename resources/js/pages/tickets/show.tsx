...
export default function TicketShow() {
  const { auth, ticket, statuses, currentAssignment, abilities, staffOptions, assetOptions, priorityOptions, flash } = usePage<PageProps>().props;
  ...
  React.useEffect(() => {
    if (abilities.canRate && !abilities.hasRated && !(ticket as any).rating && ticket.status === 'ticket_approved') {
      setIsRatingModalOpen(true);
      setSuccessModal({ open: false });
      return;
    }
            ...
            <Card className="border-none shadow-md">
              ...
              <CardContent className="pt-6">
                <div className="relative space-y-4 before:absolute before:inset-y-0 before:left-[17px] before:w-0.5 before:bg-muted">
                  {(() => {
                    const activityLogs = (ticket.activity_logs || []).filter(
                      (log) => log.action !== 'chat_comment' && log.action !== 'commented'
                    );
...
function TicketChatSection({ ticket, auth }: { ticket: any; auth: any }) {
  ...
  return (
    <Card className="overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm rounded-xl bg-white dark:bg-slate-900">
      {/* Header */}
      <CardHeader className="bg-slate-50 dark:bg-slate-800/60 text-slate-800 dark:text-slate-100 p-2.5 px-4 flex flex-row items-center justify-between border-b border-slate-200/80 dark:border-slate-800">
        <div className="flex items-center gap-2">
        ...
        <Badge variant="outline" className="text-[10px] px-2 py-0.5 border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-medium">
          Telegram Synced
        </Badge>
      </CardHeader>
      ...
      <CardContent className="p-0">
        <div
          ref={chatScrollRef}
          className="h-[220px] overflow-y-auto p-3 space-y-2 bg-slate-50/50 dark:bg-slate-950/40"
        >
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-4 text-slate-400">
              ...
              <p className="text-xs font-medium text-slate-500">No comments yet. Type a message below to start.</p>
            </div>
          ) : (
            messages.map((log) => {
              const isOwn = (log.user_id && currentUserId && (Number(log.user_id) === Number(currentUserId))) || log.user?.name === 'Me';
              const senderName = isOwn ? 'You' : (log.user?.name || 'User');
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-lg px-3 py-1.5 text-xs shadow-none border ${
                      isOwn
                  ...
                  >
                    {!isOwn && (
                      <span className="block font-semibold text-[10px] text-blue-600 dark:text-blue-400 mb-0.5">
                        {senderName}
                      </span>
                    )}
                    <p className="whitespace-pre-wrap break-words">{log.reason}</p>
                    <div className={`text-[9px] text-right mt-0.5 ${isOwn ? 'text-blue-100' : 'text-slate-400'}`}>
                      {formattedTime}
                    </div>
                  </div>
                </div>
        ...
        <form
          ...
          className="p-2 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center gap-2"
        >
          <Input
            value={inputMessage}
            onChange={(e) => setInputMessage(e.target.value)}
            placeholder="Type a message..."
            className="h-8 text-xs rounded-md border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 focus-visible:ring-blue-500 flex-1"
          />
          <Button
            type="submit"
            disabled={!inputMessage.trim()}
            size="sm"
            className="h-8 px-3 rounded-md bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs flex items-center gap-1 shrink-0"
          >
