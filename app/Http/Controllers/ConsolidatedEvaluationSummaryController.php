...
class ConsolidatedEvaluationSummaryController extends Controller
{
    public function summary(Request $request)
    {
        ...
        [$result, $staffNames] = $this->computeSummaryRows($branchId, $periodId);

        return Inertia::render('reports/consolidated-evaluation-summary', [
            'rows' => $result,
    ...
    public function export(Request $request)
    {
        ...
        [$rows, $staffNames] = $this->computeSummaryRows($branchId, $periodId);

        $columnsParam = (string) $request->query('columns', '');
        $visibleNames = $staffNames;
        if ($columnsParam !== '') {
            $requested = array_map('trim', explode(',', $columnsParam));
            $visibleNames = array_values(array_intersect($staffNames, $requested));
        ...
        $headers = [
            ...
            'Content-Disposition' => 'inline; filename="consolidated-evaluation-summary.csv"',
    ...
    {
        // Select all Branch Manager -> [Regional/Maintenance/Productions] evaluation components
        ...
        // Gather the responses mapped with Evaluators and Evaluatee (Staff) detail
        ...
        if (!empty($responseIds)) {
            ...
            foreach ($evaluationResponses as $response) {
                ...
                if ($avgScore !== null) {
                    $key = $response->evaluator_id . '|' . $response->staff_name;
        ...
        $staffNames = $evaluationData->pluck('staff_name')->unique()->sort()->values()->toArray();

        $evaluatorScores = $evaluationData
            ->groupBy('evaluator_id')
            ->map(function ($group) {
                return $group->pluck('avg_score', 'staff_name')->map(function ($score) {
        ...
        foreach ($evaluators as $evaluator) {
            ...
            foreach ($staffNames as $sName) {
                $pivot[$key][$sName] = $scores[$sName] ?? null;
            }
        }
    ...
    /**
     * Get detailed evaluation response data for a specific eval/staff map
     */
    public function details(Request $request)
    {
        ...
        if (!$evaluatorId || !$staffName || !$periodId) {
        ...
        try {
            $responses = DB::table('evaluation_responses as er')
                ...
                ->whereRaw('CONCAT(staff.first_name, " ", staff.last_name) = ?', [$staffName])
